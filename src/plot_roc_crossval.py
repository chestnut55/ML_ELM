...
X, Y = data.colorectal_cancer_data()
...
for x in params:
    ...
    for train, test in cv.split(X, Y):
        if isinstance(x[0], gcForest):
            _ = x[0].cascade_forest(X.iloc[train].values, Y.iloc[train].values)
            # x[0].fit(X.iloc[train].values, Y.iloc[train].values)
            y_probas = x[0].cascade_forest(X.iloc[test].values)
            # Compute ROC curve and area the curve
            tmp = np.mean(y_probas, axis=0)
            preds = np.argmax(tmp,axis=1)
            fpr, tpr, thresholds = roc_curve(Y.iloc[test].values, preds)
