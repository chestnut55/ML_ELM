...
def conv_net_classifier():
    ...
    L4 = 256  # number of neurons for dense2 layer
    ...
    x_shape = tf.reshape(xs, [-1, 1, num_input, 1])
    ###############autoencoder#########################################################
    ...
    tmp_shape = (int)(math.ceil(num_input/4.0))
    ...
    # fourth layer, output
    w_fc2 = weight_variable([L3, L4])
    b_fc2 = bias_variable([L4])
    h_fc2 = tf.nn.relu(tf.matmul(h_fc1, w_fc2) + b_fc2)
    h_fc2_drop = tf.nn.dropout(h_fc2, keep_prob)

    # fifth layer, output
    w_fc3 = weight_variable([L4, classes])
    ...
    train_features_cnn = np.zeros((len(train_data), L4), dtype=float)
    train_labels_cnn = np.zeros(len(train_data), dtype=int)
    test_labels_cnn = np.zeros(len(test_data), dtype=int)

    fc2_features_cnn = np.zeros((len(train_data)*2, L4), dtype=float)
    fc2_labels_cnn = np.zeros(len(train_data)*2, dtype=int)
    ...
    for i in range(len(train_data)):
        _, l = sess.run([optimizer, loss], feed_dict={X: train_data.iloc[i].reshape(1,num_input)})
        # display logs per step
        if i % 100 == 0 or i == 0:
            print('Step %i: Loss: %f' % (i, l))

    train_features_ace = encoder_op.eval(feed_dict={X: train_data})
    train_labels_ace = train_labels
    test_features_ace = encoder_op.eval(feed_dict={X: test_data})
    test_labels_ace = test_labels
    ##############################################
    ...
    for epoch in range(epochs):
        for batch in range(len(train_data) // batch_size):
            ...
            features_batch = h_fc2.eval(feed_dict={xs: train_batch_data})
    ...
    test_features_cnn = h_fc2.eval(feed_dict={xs: test_features_ace})
    ...
    aa = np.concatenate([train_features_cnn,train_features_ace])
    bb = np.concatenate([train_labels_cnn,train_labels_cnn])
    clf.fit(aa, bb)
